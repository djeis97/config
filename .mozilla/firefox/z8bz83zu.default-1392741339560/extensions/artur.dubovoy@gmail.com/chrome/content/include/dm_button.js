	...
	FVD_SINGLE_DM_Button = function(  ){
		...
		this.insert = function( url )	{	
			...
			if( !doc.getElementById(this.BUTTON_ID) ){
				...
				var elemFlag_end = block.querySelector('.views_and_share');
				//elemFlag_end.setAttribute("style", "width:230px;");
				var elemFlag_start = block.querySelector('.media');
				elemFlag_start.setAttribute("style", "width:370px; overflow: visible");
				
				var elemFlag = elemFlag_start;
				if( elemFlag_start )	{
					var elemFlag_foreground = block.querySelector('.media-block');
					elemFlag_foreground.setAttribute("style", "float:left; width: 170px;");
					var dv = doc.createElement("div");
					dv.appendChild( button );	
					elemFlag_start.appendChild( dv );					
		...
		this.createButtonElementDM = function( buttonId )	{	
			...
			div2.setAttribute("style",  "float:left;  margin-top: -5px;");					
		...
		this.createSetLi = function( menu, type )  {
			...
			if (type == 'formats')	{
				...
				var img = doc.createElement("img");
				img.setAttribute("align","left");
				img.setAttribute("src", EXT_IMAGES["nim"]);
				span.appendChild(img);
