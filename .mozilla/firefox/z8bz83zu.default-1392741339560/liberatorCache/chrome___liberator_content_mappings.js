...
const Mappings = Module("mappings", {
    ...
    list: function (modes, filter, urlPattern) {
        ...
        for (map of maps) {
            ...
            let option = xml({raw: [""],cooked: [""]}, []);
            var add = function (lhs, rhs) xml({raw: ["", "", ""],cooked: ["", "", ""]}, [(lhs), (rhs)]);
            if (map.silent)
                option = add(option, xml({raw: ["<span highlight=\"Keyword\">silent</span>"],cooked: ["<span highlight=\"Keyword\">silent</span>"]}, []));
            if (map.noremap) {
                if (map.silent)
                    option = add(option, ", ");
                option = add(option, xml({raw: ["<span highlight=\"Keyword\">noremap</span>"],cooked: ["<span highlight=\"Keyword\">noremap</span>"]}, []));
            }
