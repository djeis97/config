...
return {
    ...
    { rule = { class = JACK_CONTROL_CLASS },
      properties = { tag=tags[3][8] } },
    { rule = { class = FIREFOX_CLASS },
      properties = { tag=tags[1][4], border_width="0" } },
    { rule = { class = LIBREOFFICE_CLASS },
      callback = function (c) modality.registerClient(c,  LIBREOFFICE_KEYSTBL) end
     },
    { rule = { class = GLOBULATION_CLASS },
      callback = function (c) modality.registerClient(c, GLOBULATION_KEYSTBL) end}
}
